...
- Parser rules (sigil strictness): MUST treat `<pass://` as the only valid placeholder sigil and MUST reject any whitespace between `pass` and `://` and any legacy token that begins with `<pass:` but does not continue with `//`; diagnostics including line and column follow Sections 4.3 and 4.5.
